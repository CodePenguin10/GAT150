...
namespace nu
{
	struct Vector3
	{
		...
		float Length() const { return std::sqrt(x * x) + std::sqrt(y * y) + std::sqrt(z * z); }
		Vector3 Normalized() const { return (*this) / Length(); }
		float Dot(const Vector3& v) const { return (this->x * v.x) + (this->y * v.y) + (this->z * v.z); }

		const float Dot(const Vector3& v1, const Vector3& v2) const { return (v1.x + v2.x) + (v1.y + v2.y) + (v1.z + v2.z); }
	};
